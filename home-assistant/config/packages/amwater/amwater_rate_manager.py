#!/usr/bin/env python3
"""
Missouri American Water Rate Manager (St Louis County)
Downloads, parses, and updates residential water rates.

Usage:
    python3 amwater_rate_manager.py [--download] [--parse] [--check]
"""
...
AMWATER_URLS = {
    "mo_tariff": "https://amwater.com/moaw/resources/PDF/Customer-Service/Water-Sewer-Rates/missouri-amwater-water-rates-schedule.pdf",
}

CACHE_DIR = Path(__file__).parent
CACHE_FILES = {
    "mo_tariff": CACHE_DIR / "missouri-amwater-water-rates-schedule.pdf",
}
...
class AmwaterRateParser:
    """Parse Missouri American Water rate sheets from PDF files."""
    ...
    def parse_rates(self) -> Dict[str, Any]:
        """Parse water rates from the extracted PDF text.

        Returns:
            Dictionary with effective_date, customer_charge, and commodity_rate_per_100_gal.
        """
        rates: Dict[str, Any] = {
            "effective_date": None,
            "customer_charge": None,
            "commodity_rate_per_100_gal": None,
        }

        date_match = re.search(r"Effective Date:\s*(\w+\s+\d{1,2},\s+\d{4})", self.text)
        if date_match:
            rates["effective_date"] = date_match.group(1)

        # Rate A (St Louis County Service Area) minimum customer charge
        customer_match = re.search(r"5/8\"\s*\$?(\d+\.\d{2})", self.text)
        if customer_match:
            rates["customer_charge"] = float(customer_match.group(1))

        # Commodity charge per 100 gallons
        commodity_match = re.search(
            r"Commodity Charge.*?\$?(\d+\.\d{5})\s*per\s*100\s*gallons",
            self.text,
            re.IGNORECASE | re.DOTALL,
        )
        if commodity_match:
            rates["commodity_rate_per_100_gal"] = float(commodity_match.group(1))

        return rates


class AmwaterRateManager:
    """Manage Missouri American Water rates: download, parse, and update Home Assistant."""
    ...
    def download_pdfs(self) -> bool:
        """Download latest rate PDFs from Missouri American Water.

        Returns:
            True if all downloads succeeded, False otherwise.
        """
        success = True
        for name, url in AMWATER_URLS.items():
    ...
    def parse_rates(self) -> Optional[Dict[str, Any]]:
        ...
        """
        pdf_path = CACHE_FILES["mo_tariff"]
        if not pdf_path.exists():
            logger.error("AMWater tariff PDF not found: %s", pdf_path)
            return None
        parser = AmwaterRateParser(pdf_path)
    ...
    def update_home_assistant(self, dry_run: bool = True) -> bool:
        """Update Home Assistant input_number helpers with parsed rates.

        Args:
            dry_run: If True, log updates without sending to Home Assistant.
        ...
        updates = {
            "input_number.amwater_mo_stl_monthly_fixed": self.rates.get(
                "customer_charge"
            ),
            "input_number.amwater_mo_stl_rate_per_100_gal": self.rates.get(
                "commodity_rate_per_100_gal"
            ),
        }
        ...
        try:
            ...
            for entity_id, value in updates.items():
                ...
                data = {
                    "state": str(value),
                    "attributes": {
                        "updated_by": "amwater_rate_manager",
    ...
    def print_rate_summary(self) -> None:
        ...
        if not self.rates:
            logger.error("No rates loaded")
            return
        logger.info("MISSOURI AMERICAN WATER (ST LOUIS COUNTY) RATES")
        ...
        logger.info(
            "Commodity Charge: $%.5f per 100 gal",
            self.rates.get("commodity_rate_per_100_gal"),
        )


def main() -> None:
    """Main entry point for rate manager CLI."""
    parser = argparse.ArgumentParser(description="MO American Water Rate Manager")
    ...
    manager = AmwaterRateManager(force_download=args.force_download)
